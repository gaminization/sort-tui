from __future__ import annotations
import math
from typing import Generator, List, Any
from sortui.algorithms.base import SortAlgorithm, SortFrame
from sortui.algorithms._helpers import base_frame, done_frame
from sortui.algorithms._helpers import out_of_order, value_of, is_sorted, in_order



def _base_frame(arr, **kwargs):
    kwargs.setdefault('explanation', 'Sorting step')
    kwargs.setdefault('operation', 'compare')
    return base_frame(arr, **kwargs)

class MaxSort(SortAlgorithm):
    name = "Max Sort"
    category = "CATEGORY"  # replaced by init
    time_complexity = "O(n²)"
    space_complexity = "O(1)"
    stable = False
    description = "Selection sort variant finding the maximum."
    
    def get_invariant(self) -> str:
        return "The suffix arr[boundary..n-1] contains the boundary largest elements in their final sorted positions."


    def sort(self, arr: List[int], ascending: bool = True) -> Generator[SortFrame, None, None]:
        n = len(arr)
        if n <= 1:
            yield done_frame(arr, self.name)
            return
        sorted_idx = []
        for i in range(n - 1, 0, -1):
            max_idx = i
            for j in range(i - 1, -1, -1):
                yield _base_frame(arr, highlighted=[max_idx, j], sorted_indices=sorted_idx)
                if out_of_order(arr[j], arr[max_idx], ascending):
                    max_idx = j
            if max_idx != i:
                arr[i], arr[max_idx] = arr[max_idx], arr[i]
                yield _base_frame(arr, swapped=[i, max_idx], sorted_indices=sorted_idx, operation="swap")
            sorted_idx.append(i)
        yield done_frame(arr, self.name)

