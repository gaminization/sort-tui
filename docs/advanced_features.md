...
sort-tui --benchmark bubble insertion quicksort --size 500 --seed 42

# Export benchmark results to a JSON file and plot them (if supported)
sort-tui --benchmark bubble quicksort --benchmark-export results.json --complexity-plot
```
...
```bash
sort-tui --replay ~/sortui_run_20260101_120000.json
```
...
```bash
sort-tui --compare bubble insertion quicksort
```
