...
### Changed
- **Repository Restructuring**: Cleaned up the repository root to be more professional.
- Moved `CONTRIBUTING.md` and `CODE_OF_CONDUCT.md` into the `.github/` folder to reduce top-level clutter.
- Removed unused and obsolete boilerplate files (`ROADMAP.md`, `SECURITY.md`).
...
### Fixed
- HOTFIX: Restored documentation files that were accidentally truncated during the `sort-tui` rebranding sweep.

## [0.1.4] - 2026-07-22

### Changed
- Unified the command-line interface! The legacy `sortui` command has been removed in favor of `sort-tui` to match the project name exactly.
- All documentation, help menus, CLI arguments, and config directories (`~/.config/sort-tui`) have been fully migrated to use the `sort-tui` branding.
...
[0.1.7]: https://github.com/gaminization/sort-tui/releases/tag/v0.1.7
